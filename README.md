...
```
              ...
              └──┬───────────┬──┘         └─────────────────┘
                 │           │
       ┌─────────▼──┐  ┌─────▼─────────────┐
       │  Groq      │  │  HF Inference API  │
       │  Llama-3.3 │  │  BAAI/bge-small-en │
       │  Whisper   │  │  (CPU, 384-dim)    │
       └────────────┘  └────────────────────┘
```
...
**Shared-budget LLM routing.** Both user-waiting paths (tutor, on-demand summaries) and background batch artifact generation hit **Groq** (`llama-3.3-70b-versatile`). Since they now share one rate-limit budget, `generate_job`'s fan-out is capped by a `GROQ_BATCH_MAX_CONCURRENCY=2` semaphore so bursts of ingest calls don't starve latency-sensitive requests.
...
- **LLM provider** — Groq for both human-waiting paths and background batch generation (`llama-3.3-70b-versatile`, `whisper-large-v3`); batch fan-out is semaphore-capped (`GROQ_BATCH_MAX_CONCURRENCY`) since it shares Groq's rate limit with live requests
...
# 1. env
cp .env.example .env
# fill in: GROQ_API_KEY, SUPABASE_URL,
...
**Ingest pipeline.** Upload hits `POST /api/ingest`; a `parse_job` extracts text from the source, semantically chunks it, embeds each chunk via the HuggingFace Inference API into `chunks.embedding`, and then chains into `generate_job`. The generator runs all artifact types in parallel — but Groq calls go through a `GROQ_BATCH_MAX_CONCURRENCY=2` semaphore, since background generation shares Groq's rate limit with live human-waiting requests. Each artifact persists the moment its LLM call returns, so the feed populates incrementally. Status streams to the client via SSE on `GET /api/ingest/{doc_id}/status`.
...
- `GROQ_BATCH_MAX_CONCURRENCY` — caps concurrent background-generation calls so ingest bursts don't starve latency-sensitive requests sharing the same Groq rate limit
