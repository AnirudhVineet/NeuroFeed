  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-32 pt-24">
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-white/55">Feed</p>
          <h1 className="text-2xl font-bold text-white">What everyone's learning</h1>
        </div>
        <button
      ...
      <div className="mt-3 flex flex-wrap gap-1 rounded-full border border-white/10 bg-white/[0.04] p-1">
        {FILTERS.map((f) => (
          <button
            key={f.id}
      ...
      <ul className="mt-4 space-y-2">
        {!loading && !err && rows.length === 0 && (
          <li className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-sm text-white/55">
          ...
          <li key={r.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-3">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1 text-sm text-white/90">
                <p>
                  <Link to={`/u/${r.actor_username}`} className="font-semibold text-white hover:text-primary-soft">
                    @{r.actor_username}
                  </Link>{' '}
                  <span className="text-white/75">{r.verb}</span>{' '}
                  <span className="font-semibold text-white">{r.object_text}</span>
                </p>
                <p className="mt-0.5 text-[10px] text-white/45">{relTime(r.ts)}</p>
              </div>
            </div>
          </li>
        ))}
