...
export default function LeaderboardPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 pb-32 pt-24">
      <SocialChips />
      <header>
        <p className="text-[10px] uppercase tracking-widest text-white/55">Compete</p>
        <h1 className="text-2xl font-bold text-white">Leaderboard</h1>
        <p className="mt-1 text-sm text-white/65">Ranked by XP across NeuroFeed.</p>
      </header>
      ...
      <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
        ...
        {scope === 'subject' ? (
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-xs text-white/65">
            <p className="text-[10px] uppercase tracking-widest text-white/55">Your rank</p>
            <p className="mt-1 text-xl font-bold tabular-nums text-white">{myRank ? `#${myRank}` : '—'}</p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-xs text-white/65">
          <p className="text-[10px] uppercase tracking-widest text-white/55">Your XP</p>
          <p className="mt-1 text-xl font-bold tabular-nums text-white">{(social.profile?.xp ?? 0).toLocaleString()}</p>
        </div>
      </div>
      ...
      {!loading && !err && rows.length === 0 && (
        <p className="mt-6 rounded-2xl border border-dashed border-white/10 p-6 text-center text-sm text-white/55">
        ...
        {!err && rows.map((u, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2 text-[11px] tabular-nums text-white/85">
...
function Stat({ label, value }: { label: string; value: string | number }) {
  return (
    <span className="rounded-full bg-white/[0.04] px-2 py-0.5 text-white/85">
      <span className="mr-1 text-[9px] uppercase tracking-widest text-white/45">{label}</span>{value}
    </span>
  );
}
